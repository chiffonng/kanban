...
const UserPage = () => {
		...
		if (title) {
			try {
				...
				if (response.data && response.data.id) {
					setLists([
						...lists,
						{ id: response.data.id, name: title, tasks: [] },
					]);
				} else {
				...
				if (
					...
					window.alert(
						"There was an issue creating the list. Please try again."
					);
				}
			}
		}
	...
	const removeList = async (listId) => {
		...
		try {
			...
			const response = await axios.delete(
				`http://127.0.0.1:5000/api/list/${listId}`,
				{
					headers: {
						Authorization: `Bearer ${user.token}`,
					},
		...
		if (source.droppableId !== destination.droppableId) {
			...
			try {
				const response = await axios.put(
					`http://127.0.0.1:5000/api/task/${taskToMove.id}/move/${destList.id}`,
					{},
					{
						headers: {
	...
	const createTask = async (listId, title, parentId = null, callback) => {
		...
		try {
			const response = await axios.post(
				`http://127.0.0.1:5000/api/task/add/${listId}`,
				{
					title: newTask.title,
					parent_id: parentId,
				},
				...
				const removeTaskFromChildren = (tasks) => {
					return tasks.reduce((acc, task) => {
						if (task.id === taskId) {
							...
							const updatedChildren = removeTaskFromChildren(
								task.children || []
							);
	...
	const updateListTitle = async (listId, newTitle) => {
		try {
			...
			if (
				...
				window.alert(
					"There was an issue updating the list title. Please try again."
				);
			}
		}
	};
				...
				{user ? (
					...
					<div className="flex justify-center items-center mt-20">
						<p className="text-lg text-gray-700">
							Log in to start managing your todo lists
						</p>
					</div>
				)}
			</div>
